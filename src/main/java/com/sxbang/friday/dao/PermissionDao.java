...
public interface PermissionDao {
    ...
    @Select("select * from sys_permission t where t.type='1' limit #{startPosition},#{limit} ")
    List<SysPermission> findNavigation(@Param("startPosition") Integer page, @Param("limit") Integer limit);

    @Select("select count(*) from sys_permission t where t.type='1'")
    Long findNavigationCount();

    @Select("select * from sys_permission t where t.type='2' limit #{startPosition},#{limit} ")
    List<SysPermission> findMenu(Integer startPosition,Integer limit);
